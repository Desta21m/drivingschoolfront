...
import DeleteCarRent from '../../DeletePopups/DeleteCarRent';
...
const CarRents = () => {
  ...
  useEffect(() => {
    const fetchCarRents = async () => {
      try {
        const response = await axios.get('http://localhost:8080/api/rented-cars'); // Update the endpoint as needed
      ...
      <aside id="default-sidebar" className="fixed top-0 left-0 z-40 w-64 h-screen transition-transform -translate-x-full sm:translate-x-0" aria-label="Sidenav">
        <div className="overflow-y-auto py-5 px-3 h-full  border-r  bg-gray-800 border-gray-700">
          <ul className="space-y-2">
            <li>
              <Link to="/CarRents" className="flex items-center p-2 text-base font-normal text-blue-700 rounded-lg dark:text-white hover:bg-gray-100 dark:hover:bg-gray-700 group">
            ...
            <table className="w-full text-sm text-left text-gray-500 dark:text-gray-400">
              <thead className="text-xs text-gray-700 uppercase bg-gray-50 dark:bg-gray-700 dark:text-gray-400">
                <tr>
                  ...
                  <th scope="col" className="px-6 py-3">Rental Start Date</th>
                  <th scope="col" className="px-6 py-3">Rental End Date</th>
                  <th scope="col" className="px-6 py-3">PRICE</th>
                  <th scope="col" className="px-6 py-3">Status</th>
                </tr>
              </thead>
              <tbody>
                {carRents[0] && carRents.map((rent, index) => (
                  <tr key={index} className="bg-white border-b dark:bg-gray-800 dark:border-gray-700">
                    <td className="px-6 py-4 font-medium text-gray-900 whitespace-nowrap dark:text-white">{rent.renter.firstName} {rent.renter.lastName}</td>
